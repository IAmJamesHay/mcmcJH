...
plot_model_fits_publication<- function(model_data,
                                       actual_data,
                                       group,
                                       plot_bounds,
                                       lower=NULL,
    ...
    xscale <- infection_times$time
    xlabels <- infection_times$infection
    xlabel_colours <- rep("red",nrow(infection_times))
    xlabel_sizes <- rep(16,nrow(infection_times))
                            ...
                            scale_y_continuous(breaks=seq(low,high,by=1),limits=c(low,high),expand=c(0,0))+
                                ggtitle(title)+
                                        theme(
                                            ...
                                            text=element_text(size=16,colour="gray12"),
                                            plot.title=element_text(size=24),
                                            legend.text=element_text(size=16,colour="gray12"),
                                            panel.grid.major = element_blank(),
                                            panel.grid.minor = element_blank(),
                                            axis.line=element_line(colour="gray12"),
                                            axis.line.x = element_line(colour = "white"),
                                            axis.line.y=element_line(colour="gray12"),
                                            ...
                                            geom_hline(yintercept=0,colour="gray25",linetype="dotted") +
                                                scale_x_continuous(breaks=xscale,labels=xlabels) + theme(axis.text.y=element_text(size=16,colour="gray12")) +
                                                    ylab("") +
                                                        xlab("")
    
    if(!is.null(ylabel_text)) plot <- plot + ylab(ylabel_text)
    if(bottom_infection_labels){
        xscale1 <- c(0,21,37,49,70, infection_times$time)
        xlabels1 <- c("0","21","37","49","70",paste("\n\n",infection_times$infection,sep=""))
        xlabel_colours1 <- c(rep("gray12",5),rep("red",nrow(infection_times)))
        xlabel_sizes1 <- c(rep(16,5),rep(16,nrow(infection_times)))
        plot <- plot + scale_x_continuous(breaks=xscale1,labels=xlabels1) + 
            theme(axis.text.x=element_text(colour=xlabel_colours1,size=xlabel_sizes1))
    }

    if(xline) plot <- plot + theme(axis.line.x=element_line(colour="gray20"))
    if(xlabel) plot <- plot + xlab("Time (days")
